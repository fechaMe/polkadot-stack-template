		...
		<div className="space-y-6 animate-fade-in max-w-lg mx-auto">
			<div className="space-y-1">
				...
				<p className="text-text-secondary text-sm">
					Transfer <span className="font-mono text-text-primary">{id}</span> · Paseo Asset
					Hub + Bulletin Chain IPFS
				</p>
			</div>
				...
				<div className="mt-3 flex gap-2">
					<input
						...
						onChange={(e) => {
							...
							if (addr)
								localStorage.setItem(`${CONTRACT_STORAGE_KEY}:${ethRpcUrl}`, addr);
						}}
						placeholder="0x…"
			...
			{loadError && !loading && (
				<div className="card space-y-3">
					...
					<button onClick={loadTransfer} className="btn-secondary text-xs">
						Retry
					</button>
				</div>
			)}
			...
			{transfer && !loading && (
				<div className="card space-y-5">
					...
					{!isRevoked && isExpired && (
						<div className="rounded-lg bg-accent-red/10 border border-accent-red/20 px-3 py-2 text-sm text-accent-red">
							This transfer has expired. The file may no longer be on the Bulletin
							Chain.
						</div>
					)}
					...
					<div className="grid grid-cols-2 gap-2 text-xs">
						<div className="rounded-lg border border-white/[0.04] bg-white/[0.02] p-2.5">
							...
							<p className="text-text-secondary font-mono">
								{shortAddress(transfer.uploader)}
							</p>
						</div>
						<div className="rounded-lg border border-white/[0.04] bg-white/[0.02] p-2.5">
							...
							<p
								className={
									isUnavailable ? "text-accent-red" : "text-text-secondary"
								}
							>
							...
							{dlState.total > 1 && (
								<div className="h-1 rounded-full bg-white/[0.05] overflow-hidden">
									<div
										...
										style={{
											width: `${(dlState.fetched / dlState.total) * 100}%`,
										}}
									/>
								</div>
							)}
					...
					{dlState.type === "done" && (
						<div className="space-y-4">
							...
							<a
								...
								style={{
									background: "linear-gradient(135deg, #00c8ff 0%, #0098c4 100%)",
									color: "#060b14",
									fontWeight: 600,
								}}
							>
